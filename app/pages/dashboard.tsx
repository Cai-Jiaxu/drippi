...
const getStatusColor = (status: string) => {
  switch (status.toLowerCase()) {
    ...
    case 'cancelled': return '' // Do not display cancelled rentals
...
export default function Dashboard() {
  ...
  const handleCancelRental = async (rentalId: number) => {
    ...
    if (res.ok) {
      ...
      // Remove the rental from the selected listing if it exists
      if (selectedListing) {
        setSelectedListing({
          ...selectedListing,
          rentals: selectedListing.rentals?.filter(r => r.id !== rentalId)
        })
      }
    } else {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white p-4 md:p-8 transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-center gap-4 mb-8">
          <button
            className={`px-4 py-2 rounded-lg border transition-colors ${activeTab === 'lister' ? 'bg-purple-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700'}`}
            onClick={() => setActiveTab('lister')}
          >
            Your Listings
          </button>
          <button
            className={`px-4 py-2 rounded-lg border transition-colors ${activeTab === 'renter' ? 'bg-purple-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700'}`}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {listings.map(listing => (
                  ...
                  >
                    ...
                    <span className={`px-2 py-1 mt-1 inline-block text-xs rounded ${getStatusColor(listing.rentals?.[0]?.status ?? 'available')}`}>
                      {listing.rentals?.[0]?.status?.toUpperCase() ?? 'AVAILABLE'}
                    </span>
                  </div>
                ))}
              </div>
            ...
            {selectedListing && (
              <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50">
                <div className="bg-white dark:bg-gray-900 p-6 w-full max-w-2xl rounded-lg relative max-h-[90vh] overflow-y-auto shadow-xl">
                  ...
                  {/* Display rental status outside modal */}
                  <p className="text-sm text-gray-500 mb-4">
                    Rental Status: 
                    {selectedListing?.rentals?.[0]?.status === 'cancelled' ? '' : selectedListing?.rentals?.[0]?.status?.toUpperCase() || 'No rentals yet'}
                  </p>

                  {selectedListing.rentals?.map(r => (
                    r.status !== 'cancelled' && ( // Do not display cancelled rentals
        ...
        {activeTab === 'renter' && (
          <>
            {rentals.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {rentals.map(rental => (
                  rental.status !== 'cancelled' && ( // Do not display cancelled rentals
