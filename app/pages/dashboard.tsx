  ...
  const fetchRentals = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      const filtered = data.filter(
        (rental: Rental) => rental.status.toLowerCase() !== 'cancelled'
      )
      setRentals(filtered)
    }
          ...
          <>
            {loadingListings ? (
              ...
              <div className="text-center">
                ...
                <Link href="/upload" className="text-purple-600 underline">
                  Click here to list your first outfit.
                </Link>
              </div>
            ) : (
                    ...
                    selectedListing.rentals?.map((r) => (
                      <div key={r.id} className="border-t pt-4 mt-4 space-y-1">
                        ...
                        <p>
                          ...
                          {r.renter.profile?.telegram_handle
                            ? `@${r.renter.profile.telegram_handle}`
                            : 'N/A'}
                        </p>
                        <p>
          ...
          <>
            {loadingRentals ? (
              ...
              <div className="text-center">
                ...
                <Link href="/listings" className="text-purple-600 underline">
                  Click here to rent an outfit.
                </Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                  ...
                  >
                    ...
                    <p className="text-sm font-medium">
                      ...
                      {calculateTotalPrice(
                        rental.start_date,
                        rental.end_date,
                        rental.outfit_details.price_per_day
                      )}
                    </p>
                    <span
                      className={`px-2 py-1 mt-1 inline-block text-xs rounded ${getStatusColor(
                        rental.status
                      )}`}
                    >
                    ...
                    {rental.status === 'approved' && rental.outfit_details.owner ? (
                      <div className="mt-3 text-sm text-gray-700">
                        <h4 className="font-semibold mb-1 underline">
                          Owner Contact Details
                        </h4>
                        <p>
                          <strong>Telegram:</strong>{' '}
                          {rental.outfit_details.owner.profile?.telegram_handle
                            ? `@${rental.outfit_details.owner.profile.telegram_handle}`
                            : 'N/A'}
                        </p>
                        <p>
                          <strong>Phone:</strong>{' '}
                          {rental.outfit_details.owner.profile?.phone_number ?? 'N/A'}
                        </p>
                      </div>
